...
from nets.yolo_training import (ModelEMA, YOLOLoss,CorrelationLoss, DecoupleLoss, get_lr_scheduler,
...
from utils.utils_fit import fit_one_epoch, fit_one_epoch_decouple
import json
...
if __name__ == "__main__":
    ...
    # model_path_clip = "E:/wt/datasets/ViT-B-16-OpenAI.pth"
    # model_path = 'logs/ep100-loss0.319-val_loss0.464.pth'
    #------------------------------------------------------#
    ...
    # 新加json文件地址
    train_json_path = '/data/user7/wt/datasets/M3FD/M3FD/VOCdevkit/VOC2007/ImageSets/Main/train.json'
    val_json_path = '/data/user7/wt/datasets/M3FD/M3FD/VOCdevkit/VOC2007/ImageSets/Main/val.json'
    ...
    if model_path != '':
        ...
        # 加载第二个预训练模型的权重，但只加载那些名字匹配的参数
        # pretrained_dict_clip = torch.load(model_path_clip, map_location=device)
        # modified_dict = {}
        # for k, v in pretrained_dict_clip.items():
        #     if k.startswith("transformer") or k.startswith("token_embedding") or k.startswith(
        #             "ln_final") or k.startswith("text_projection") or k.startswith(
        #             "positional_embedding") or k.startswith("logit_scale"):
        #         new_k = "contrast." + k
        #         modified_dict[new_k] = v
        #     else:
        #         modified_dict[k] = v
        # load_key_2, no_load_key_2 = [], []
        ...
        #         load_key_2.append(k)
        #     else:
        #         no_load_key_2.append(k)
        #
        # model.load_state_dict(model_dict)
        #
        # print("Loaded keys from second pretrained model:", load_key_2)
        # print("Not loaded keys from second pretrained model:", no_load_key_2)
        #------------------------------------------------------#
        #   显示没有匹配上的Key
        #------------------------------------------------------#
        if local_rank == 0:
    ...
    # 新加jsonlines
    train_json_lines = json.load(open(train_json_path, mode='r', encoding='utf-8'))
    val_json_lines = json.load(open(val_json_path, mode='r', encoding='utf-8'))
    ...
    if True:
        ...
        #   构建数据集加载器。
        #---------------------------------------#
        train_dataset   = YoloDataset(train_lines, train_json_lines, input_shape, num_classes, anchors, anchors_mask, epoch_length=UnFreeze_Epoch, \
                                        mosaic=mosaic, mixup=mixup, mosaic_prob=mosaic_prob, mixup_prob=mixup_prob, train=True, special_aug_ratio=special_aug_ratio)
        val_dataset     = YoloDataset(val_lines, val_json_lines, input_shape, num_classes, anchors, anchors_mask, epoch_length=UnFreeze_Epoch, \
        ...
        #----------------------#
        if local_rank == 0:
            eval_callback   = EvalCallback(model, input_shape, anchors, anchors_mask, class_names, num_classes, val_lines, val_json_lines, log_dir, Cuda, \
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch_decouple(model_train, model, ema, yolo_loss, decouple_loss, correlation_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir, local_rank)
