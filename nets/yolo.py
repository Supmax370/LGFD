...
from clip.nets.clip import CLIP
...
# class DecoupleModule5(nn.Module):
#     # ---------------------------------------------------#
#     #   split
#     # ---------------------------------------------------#
#     def __init__(self, n_in):
#         super(DecoupleModule5, self).__init__()
#         self.scale_conv = Conv(n_in // 2, n_in)
#         self.invariant_conv = Conv(n_in // 2, n_in)
#
#     def forward(self, x):
#         scale_feature_map0, invariant_feature_map0 = torch.split(x, 256, dim=1)
...
class DecoupleModule4(nn.Module):
    # ---------------------------------------------------#
    #   split
    # ---------------------------------------------------#
    def __init__(self, n_in):
        super(DecoupleModule4, self).__init__()
        self.scale_conv = Conv(n_in // 2, n_in)
        self.invariant_conv = Conv(n_in // 2, n_in)

    def forward(self, x):
        scale_feature_map0, invariant_feature_map0 = torch.split(x, 128, dim=1)
        scale_feature_map = self.scale_conv(scale_feature_map0)
        invariant_feature_map = self.invariant_conv(invariant_feature_map0)
        return scale_feature_map, invariant_feature_map

class DecoupleModule3(nn.Module):
    # ---------------------------------------------------#
    #   split
    # ---------------------------------------------------#
    def __init__(self, n_in):
        super(DecoupleModule3, self).__init__()
        self.scale_conv = Conv(n_in // 2, n_in)
        self.invariant_conv = Conv(n_in // 2, n_in)
...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi, pretrained=False):
        ...
        # self.decouple5 = DecoupleModule5(n_in=512)
        self.decouple4 = DecoupleModule4(n_in=256)
        self.decouple3 = DecoupleModule3(n_in=128)
        self.contrast = CLIP(
            embed_dim=512,
        ...
        self.bn2 = nn.BatchNorm2d(num_features=2048)
        self.relu = nn.ReLU(inplace=True)

        # 40, 40, 256 = > 10, 10, 2048
        self.conv1 = nn.Conv2d(in_channels=256, out_channels=2048, kernel_size=4, stride=4, padding=0)
        # 80, 80, 128 = > 10, 10, 2048
        self.conv2 = nn.Conv2d(in_channels=128, out_channels=512, kernel_size=4, stride=2, padding=1)
        self.conv3 = nn.Conv2d(in_channels=512, out_channels=2048, kernel_size=4, stride=2, padding=0)
        # 20, 20, 512 = > 10, 10, 2048
        self.conv5 = nn.Conv2d(in_channels=512, out_channels=2048, kernel_size=3, stride=2, padding=1)
        """
        self.Adaptivepool = nn.AdaptiveAvgPool2d(output_size=(7, 7))  # 输出形状: (2048, 7, 7)

        self.Conv3 = nn.Conv2d(in_channels=128, out_channels=2048, kernel_size=1)
        self.Conv4 = nn.Conv2d(in_channels=256, out_channels=2048, kernel_size=1)
    ...
    def forward(self, x, captions):
        ...
        # ------------------------尺度不变特征解耦模块开始------------------------#
        # 解耦P4
        # 40, 40, 256 = > 40, 40, 256 & 40, 40, 256
        object_feature4, noise_feature4 = self.decouple4(P4)
        # 40, 40, 256 = > 40, 40, 2048
        x4 = self.Relu(self.BN(self.Conv4(object_feature4)))
        # 40, 40, 2048 = > 7, 7, 2048
        x4 = self.Adaptivepool(x4)
        logits_per_image4,_ = self.contrast(x4, captions)

        # 解耦P3
        # 80, 80, 128 = > 80, 80, 128 & 80, 80, 128
        object_feature3, noise_feature3 = self.decouple3(P3)
        # 80, 80, 128 = > 80, 80, 2048
        x3 = self.Relu(self.BN(self.Conv3(object_feature3)))
        # 80, 80, 2048 = > 7, 7, 2048
        x3 = self.Adaptivepool(x3)
        logits_per_image3, _ = self.contrast(x3, captions)
        ...
        object_feature3, noise_feature3 = self.decouple3(P3)
        # 80, 80, 128 = > 7, 7, 2048
        x3 = self.relu(self.bn1(self.conv1(object_feature3)))
        ...
        object_feature4, noise_feature4 = self.decouple4(P4)
        # 40, 40, 256 = > 7, 7, 2048
        x4 = self.relu(self.bn1(self.conv3(object_feature4)))
        x4 = self.relu(self.bn2(self.conv4(x4)))
        x4 = self.Adaptivepool(x4)
        # 7, 7, 2048 输入
        logits_per_image4, _ = self.contrast(x4, captions)

        # # 解耦P5
        # # 20, 20, 512 = > 20, 20, 512 & 20, 20, 512
        # object_feature5, noise_feature5 = self.decouple5(P5)
        # # 20, 20, 512 = > 7, 7, 2048
        # x5 = self.relu(self.bn2(self.conv5(object_feature5)))
        # x5 = self.Adaptivepool(x5)
        # # 7, 7, 2048 输入
        # logits_per_image5, _ = self.contrast(x5, captions)
        """
        initial_feature = [initial_P3, initial_P4]
        object_feature = [object_feature3, object_feature4]
        noise_feature = [noise_feature3, noise_feature4]
        logits_per_image = [logits_per_image3, logits_per_image4]
        ...
        P3 = self.rep_conv_1(object_feature3)
        P4 = self.rep_conv_2(object_feature4)
        ...
        # 特征可视化用此行，需要可视化初始的P3、P4特征initial_feature
        # return [out0, out1, out2], object_feature, noise_feature, logits_per_image, initial_feature
        return [out0, out1, out2], object_feature, noise_feature, logits_per_image
