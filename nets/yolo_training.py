...
class CorrelationLoss(nn.Module):
    def __init__(self):
        ...
        self.avgpool = nn.AvgPool2d(kernel_size=(20, 20))

    def __call__(self, object_feature, noise_feature):
        object_feature = self.avgpool(object_feature)
        noise_feature = self.avgpool(noise_feature)
        object_feature = object_feature.view(object_feature.size(0), -1)
        noise_feature = noise_feature.view(noise_feature.size(0), -1)
        correlation_loss = F.cosine_similarity(object_feature, noise_feature, dim=1)
        correlation_loss = torch.mean(correlation_loss)
        return correlation_loss


class DecoupleLoss2(nn.Module):
    def __init__(self):
        super(DecoupleLoss2, self).__init__()
    ...
    def __call__(self, object_feature, captions, images):
        logits_per_image = self.contrast.forward(object_feature, captions)

        logits_per_text = logits_per_image.t()
        label = torch.arange(len(logits_per_image)).long().to(images.device)
        loss_logits_per_image = nn.CrossEntropyLoss()(logits_per_image, label)
        loss_logits_per_text = nn.CrossEntropyLoss()(logits_per_text, label)
        decouple_loss = (loss_logits_per_image + loss_logits_per_text) / 2.0
...
class DecoupleLoss(nn.Module):
    ...
    def __call__(self,  logits_per_image, device):
        logits_per_text = logits_per_image.t()
        label = torch.arange(len(logits_per_image)).long().to(device)
        loss_logits_per_image = self.CrossEntropyLoss(logits_per_image, label)
        loss_logits_per_text = self.CrossEntropyLoss(logits_per_text, label)
        decouple_loss = (loss_logits_per_image + loss_logits_per_text) / 2.0
