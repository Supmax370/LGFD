...
class YOLO(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        # "model_path"        : 'logs/ep230-loss0.197-val_loss0.290.pth',
        "model_path"        : r'/data/user7/wt/New_experiments/M3FD/ablation-position/YOLO-DECOUPLE-P34/logs/ep240-loss0.186-val_loss0.284.pth',
    ...
    # def generate_heatmap(self, image, caption):
    #     #---------------------------------------------------#
    #     #   建立yolo模型，载入yolo模型的权重
    #     #---------------------------------------------------#
    #     self.net    = YoloBody(self.anchors_mask, self.num_classes, self.phi)
    ...
    #         self.net = nn.DataParallel(self.net)
    #         self.net = self.net.cuda()
    #     outputs,_,_,_ = self.net(image, caption)
    #     return outputs
    ...
    def detect_heatmap(self, image, caption, heatmap_save_path):
        import cv2
        ...
        with torch.no_grad():
            ...
            #   将图像输入网络当中进行预测！
            #---------------------------------------------------------#
            outputs ,object_feature, noise_feature, logits_per_image,_ = self.net(images, caption)
        ...
        for sub_output in outputs:
            ...
            print("sub_output" + str(sub_output.shape))#(1, 27, 20, 20) (1, 27, 40, 40)(1, 27, 80, 80)
            # print("sub_output"+str(sub_output))
            sub_output = np.transpose(np.reshape(sub_output, [b, 3, -1, h, w]), [0, 3, 4, 1, 2])[0]
            print("sub_output" + str(sub_output.shape))#(20, 20, 3, 9)(40, 40, 3, 9)(80, 80, 3, 9)
            # print("sub_output" + str(sub_output))
            score      = np.max(sigmoid(sub_output[..., 4]), -1)
            print("score" + str(score.shape))#(20, 20)(40, 40)(80, 80)
   ...
   #可视化中间特征层
    def visual_featuremap(self, image, caption, heatmap_save_path):
        import cv2
        import matplotlib.pyplot as plt
        def sigmoid(x):
            y = 1.0 / (1.0 + np.exp(-x))
        ...
        with torch.no_grad():
            ...
            #   将图像输入网络当中进行预测！
            # ---------------------------------------------------------#
            outputs, object_feature, noise_feature, logits_per_image,initial_feature = self.net(images, caption)
            # object_feature3, object_feature4 = object_feature
            # noise_feature3, noise_feature4 = noise_feature
            # initial_P3, initial_P4 = initial_feature

            # 将特征层的张量转换为NumPy数组
            feature_map_np1 = initial_feature[0].cpu().detach().numpy()
            feature_map_np2 = object_feature[0].cpu().detach().numpy()
            feature_map_np3 = noise_feature[0].cpu().detach().numpy()

            # 叠加通道并显示
            combined_feature_map1 = np.sum(feature_map_np1[0], axis=0)  # 叠加第一个样本的所有通道
            combined_feature_map2 = np.sum(feature_map_np2[0], axis=0)
            combined_feature_map3 = np.sum(feature_map_np3[0], axis=0)
            combined_feature_map = [combined_feature_map1, combined_feature_map2, combined_feature_map3]
            ...
            for i, ax in enumerate(axes):
                ax.imshow(combined_feature_map[i], cmap="viridis")
    ...
    def get_map_txt(self, image_id, image, captions, class_names, map_out_path):
        ...
        with torch.no_grad():
            ...
            #   将图像输入网络当中进行预测！
            #---------------------------------------------------------#
            outputs,_,_,_ = self.net(images, captions)
            outputs = self.bbox_util.decode_box(outputs)
            #---------------------------------------------------------#
            #   将预测框进行堆叠，然后进行非极大抑制
            #---------------------------------------------------------#
